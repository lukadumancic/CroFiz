...
<?php
	if(!isset($_GET["id"])){
		header("Location: http://82.132.7.168/Forum.php");
		die();
	}
	else if($_GET["id"]==""){
		header("Location: http://82.132.7.168/Forum.php");
...
<html>
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
		...
		<script>
			...
			$(function(){
		  $('a').each(function() {
			if ($(this).prop('href') == 'http://82.132.7.168/Forum.php') {
		...
		<?php
			...
			function infoKorisnik2($id){
				...
				$conn->close();
				return "<a class='nick3 tamnije' style='font-size:17px;' href='http://82.132.7.168/Profil.php?nick=".$row["nick"]."'>".$row["ime"]." ".$row["prezime"]."</a>";
			...
			function infoKorisnik($id){
				...
				$row=$rezultat->fetch_assoc();
				$conn->close();
				return "<a class='nick3' href='http://82.132.7.168/Profil.php?nick=".$row["nick"]."'>".slika($id).$row["ime"]." ".$row["prezime"]."</a>";
