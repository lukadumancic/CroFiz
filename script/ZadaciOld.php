	...
	<body>
		...
		<article>
			...
			<div id="dnevni" class="divZadaci" style="display:none;">
				...
				<script>
					function prikaziRankListu(){window.location='http://82.132.7.168/RankLista.php';}
				</script>
			...
			<div id="tjedni" class="divZadaci" style="display:none;">
				...
				<script>
					function prikaziRankListu2(){window.location='http://82.132.7.168/RankLista2.php';}
				</script>
				<div class="ispisZadataka">
					<?php
						if(ispisiTjedni()){
		...
		<?php
			...
			function ispisiDnevni(){
				if(prijavljen()=="True"){
					...
					if($row["obrazovanje"]!="Profesor" and $row["obrazovanje"]!="Drugo"){
						...
						$row=$rez->fetch_assoc();
						$conn->close();
						return "<a style='font-size:40px;' class='zadatak' href='http://82.132.7.168/Zadatak.php?id=".$row["id"]."'>".$row["ime"]."</a>";
					}
					else{
						$conn->close();
						return False;
			...
			function ispisiSveDnevne(){
					...
					for($i=1;$i<5;$i++){
						...
						while($row=$rez->fetch_assoc()){
							$ret.="<p style='font-size:25px;font-family: serif;' >Dnevni zadatak za $i. razred</p><a style='font-size:40px;' class='zadatak' href='http://82.132.7.168/Zadatak.php?id=".$row["id"]."'>".$row["ime"]."</a><br>";
						}
					}
					$conn->close();
					return $ret;
			...
			function ispisiSveTjedne(){
					...
					for($i=1;$i<5;$i++){
						...
						$rez=$conn->query($sql);
						while($row=$rez->fetch_assoc()){
							$ret.="<p style='font-size:25px;font-family: serif;' >Tjedni zadatak za $i. razred</p><a style='font-size:40px;' class='zadatak' href='http://82.132.7.168/Zadatak.php?id=".$row["id"]."'>".$row["ime"]."</a><br>";
						}
					}
					$conn->close();
					return $ret;
			...
			function ispisiTjedan(){
				if(prijavljen()=="True"){
					...
					if($row["obrazovanje"]!="Profesor" and $row["obrazovanje"]!="Drugo"){
						...
						$ret="";
						while($row=$rez->fetch_assoc()){
							$ret.="<p class='dan'>".$row["dan"]."  <a class='zadatak' href='http://82.132.7.168/Zadatak.php?id=".$row["id"]."'>".$row["ime"]."</a></p>";
						}
						$conn->close();
					...
					if($rez->num_rows===0){
						...
						if(profesor()){
							...
							if($rez->num_rows==0){
								...
								while($row=$rez->fetch_assoc()){
									$ret.="<br><a class='grupa' href='http://82.132.7.168/Grupa.php?id=".$row["id"]."'>".$row['ime']."</a><br>Izazovi:";
									...
									if($rez2->num_rows===0){
										...
										while($row2=$rez2->fetch_assoc()){
											$ret.="<br><a class='izazov' href='http://82.132.7.168/Izazov.php?id=".$row2["id"]."'>".$row2['ime']."</a>";
			...
			function dohvatiSveZadatke($razred,$podrucje){
				$id=getId($_SESSION["korisnickoIme"]);
					...
					if ($conn->connect_error) {
						...
						$ret="";
						while($row=$rez->fetch_assoc()){
							$ret.="<a class='zadatak' href='http://82.132.7.168/Zadatak.php?id=".$row["id"]."'>".$row["ime"]."</a></p>";
						}
						$conn->close();
			...
			function unesiArhivu($razred,$podrucje){
				$servername = "82.132.7.168";
					...
					while($row=$rez->fetch_assoc()){
						echo "document.getElementById('$podrucje').innerHTML+='<a class=\"zadatak\" href=\"http://82.132.7.168/Zadatak.php?id=".$row["id"]."\">".$row["ime"]."</a><br>';";
					}
			}
		?>
	</body>
