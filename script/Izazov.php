	...
	<body>
		...
		<?php
			...
			function infoKorisnik($id){
				...
				$conn->close();
				return "<a class='nick' style='margin-left:0px;' href='http://localhost/Profil.php?nick=".$row["nick"]."'>".slika($id).$row["ime"]." ".$row["prezime"]."</a>";
			...
			function infoKorisnik2($id){
				...
				$row=$rezultat->fetch_assoc();
				$conn->close();
				return "<a class='nick3' style='margin-left:0px;' href='http://localhost/Profil.php?nick=".$row["nick"]."'>".slika($id).$row["ime"]." ".$row["prezime"]."</a>";
