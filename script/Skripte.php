		...
		<?php
			...
			function ispisSkripte(){
				if(isset($_POST["id"])){
					...
					//Ako je slika
					if(strpos($row["type"],"mage")){
						echo "<textarea class='slika' style='display: block;background-image:url(http://crofiz.com/Slike/".$_POST["id"].".jpg);' ></textarea>";
					}
					else if(strpos($row["type"],"/pdf")){
						echo "<script>";
						echo 'window.open("http://crofiz.com/Dokumenti/'.$_POST["id"].'.pdf");';
						echo "</script>";
					}
					else{
						echo "<script>";
						echo 'window.open("http://crofiz.com/Dokumenti/'.$_POST["id"].'.docx");';
						echo "</script>";
					}
				}
			}		
