...
if(prijavljen()=="True" and isset($_POST["idObjave"]) and isset($_POST["submit"])){
	...
	$sql="select * from objavetemaforum where id='".$_POST["idObjave"]."'";
	$rez=$conn->query($sql);
	if($rez->num_rows===1){
		$row=$rez->fetch_assoc();
		...
		if(!in_array($_SESSION["userId"],$korisnici)){
			...
			if($_POST["submit"]=="+1"){
				$sql="UPDATE `objavetemaforum` SET `kor`='$kor', `bodovi`=`bodovi`+1 WHERE `id`='".$_POST["idObjave"]."'";
	...
	if(prijavljen()=="True"){
		...
		$ret="";
		$sql="select * from objavetemaforum where idforum=".$_GET["id"]." order by bodovi desc";
