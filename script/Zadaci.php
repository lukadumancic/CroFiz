	...
	<meta name="viewport" content="width=device-width, initial-scale=1.0">
		...
		<?php
					...
					if($row["obrazovanje"]!="Profesor" and $row["obrazovanje"]!="Drugo"){
						...
						return 
							"<div class='opis2' style='background: url(\"Slicice/to-do.jpg\");background-size: cover;background-position: center; '>
								...
								<p><a style='font-size:40px;' class='zadatak' href='http://82.132.7.168/Zadatak.php?id=".$row["id"]."'>".$row["ime"]."</a></p>
			...
			function ispisiSveDnevne(){
					...
					for($i=1;$i<5;$i++){
						...
						while($row=$rez->fetch_assoc()){
							...
							<p>$i. razred</p>
							<p>
							<a class='zadatak' href='http://82.132.7.168/Zadatak.php?id=".$row["id"]."'>".$row["ime"]."</a>
							</p>
			...
			function ispisiSveDnevneTjedan($i){
					...
					$rez->fetch_assoc();
					while($row=$rez->fetch_assoc()){
						$ret.="<p class='dan'>".$row["dan"]."  <a class='zadatak underline' href='http://82.132.7.168/Zadatak.php?id=".$row["id"]."'>".$row["ime"]."</a></p>";
					}
								...
								while($row=$rez->fetch_assoc()){
									...
									if($rez2->num_rows===0){
										...
										while($row2=$rez2->fetch_assoc()){
											$ret.="<tr>";
											$ret.="<td><a class='izazov' href='http://82.132.7.168/Izazov.php?id=".$row2["id"]."'>".$row2['ime']."</a></td>";
			...
			function unesiArhivu($razred,$podrucje){
				$conn=conn();
					...
					while($row=$rez->fetch_assoc()){
						echo "document.getElementById('$podrucje').innerHTML+='<a style=\"display: inline;\" class=\"zadatak\" href=\"http://82.132.7.168/Zadatak.php?id=".$row["id"]."\">".$row["ime"]."</a><br>';";
						echo "document.getElementById('sve$razred').innerHTML+='<a style=\"display: inline;\" class=\"zadatak\" href=\"http://82.132.7.168/Zadatak.php?id=".$row["id"]."\">".$row["ime"]."</a><br>';";
					}
					$conn->close();
			}
			function zadnjiZadaci(){
				...
				while($row=$rez->fetch_assoc()){
					echo "<tr>";
					echo "<td><a class=\"zadatak\" href=\"http://82.132.7.168/Zadatak.php?id=".$row["id"]."\">".$row["ime"]."</a></td>';";
					...
					if($rez->num_rows===0){
						...
						if(!profesor()){
							...
							$br=0;
								...
								$ret.="<a class='izazov' href='http://82.132.7.168/izazov.php?id=".$row["id"]."'>".$row['ime']."</a>";
